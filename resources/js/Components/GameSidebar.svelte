<script>
    ...
    let { openNavigator = $bindable(false), visible = $bindable(false), ...otherProps } = $props();
</script>

{#each otherProps.gameInfo.sections as section (section.subtitle)}
    <Dropdown title={section.subtitle} link="/game/{convertSpaceToUnderscore(otherProps.gameInfo.game)}/{convertSpaceToUnderscore(section.subtitle)}" bind:visible={visible}>
